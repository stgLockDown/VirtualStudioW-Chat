...
function connectSocket(){
  ...
  // GameU Heartbeat - Keep UI in sync
  S.socket.on('heartbeat',(data)=>{
    console.log('[GameU] Heartbeat received:',data.serverTime);
    // Update connection status indicator if visible
    const statusEl = $('#connection-status');
    if(statusEl){
      statusEl.textContent = '🟢 Connected';
      statusEl.classList.remove('disconnected');
    }
    // Refresh participant list if in a meeting
    if(S.inMeeting && S.socket?.connected){
      S.socket.emit('get-participants');
    }
